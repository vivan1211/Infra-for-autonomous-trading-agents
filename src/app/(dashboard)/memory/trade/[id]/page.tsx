...
export default function TradeWikiPage() {
  ...
  const snap: any = t.data_snapshot ?? {};
  ...
  return (
    <>
      {/* Header */}
      <div className="mb-10">
        <Link href="/memory?tab=trades" className="text-[13px] text-white/60 hover:text-white/80 transition-colors">
          ← Back to Trades
        </Link>
        ...
        <div className="mt-8 border-l-2 border-[#60a5fa] pl-5 py-2">
          ...
          {autopsy.key_excerpt_agent && (
            <p className="text-[12px] text-white/70 mt-2">
              — <Link href={`/memory/agent/${autopsy.key_excerpt_agent}`} className="text-[#60a5fa] hover:underline">
        ...
        <div className="border-t border-white/[0.08] mt-3 pt-6">
          <div className="divide-y divide-white/[0.06]">
            ...
            <div className="flex items-center justify-between py-3">
              ...
              <Link href={`/memory/bot/${signals.bot_type_id || fm.bot_type_id}`} className="text-[13px] text-white hover:text-[#60a5fa] transition-colors">
                {signals.bot_type_id || fm.bot_type_id || "—"}
              </Link>
            </div>
            <div className="flex items-center justify-between py-3">
              <span className="text-[13px] text-white/80">Category</span>
              <Link href={`/memory/category/${slugify(signals.category || "")}`} className="text-[13px] text-white hover:text-[#60a5fa] transition-colors">
