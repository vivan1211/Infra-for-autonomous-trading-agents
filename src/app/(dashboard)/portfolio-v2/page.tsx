    ...
    <ResponsiveContainer width="100%" height={height}>
      <AreaChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
        ...
        <XAxis dataKey="date" axisLine={false} tickLine={false}
          tick={{ fontSize: 11, fill: "#555" }} interval="preserveStartEnd" minTickGap={40} />
        <YAxis axisLine={false} tickLine={false}
          tick={{ fontSize: 11, fill: "#555" }} width={50}
...
export default function PortfolioV2Page() {
  ...
  const { agents: apiAgents, loading: agentsLoading } = useAgents(envParam);
  ...
  const tradeStats = useMemo(() => {
    const settledTrades = allTrades.filter((t) => (t.pnl != null && t.pnl !== 0) || t.settled);
    const withPnl = settledTrades.filter((t) => t.pnl != null && t.pnl !== 0);
    ...
    let best = null as typeof withPnl[0] | null;
    let worst = null as typeof withPnl[0] | null;
    for (const t of withPnl) {
      if (!best || (t.pnl ?? 0) > (best.pnl ?? 0)) best = t;
      if (!worst || (t.pnl ?? 0) < (worst.pnl ?? 0)) worst = t;
    }

    const sortedSettled = [...settledTrades].sort((a, b) => new Date(b.settled_at || b.timestamp).getTime() - new Date(a.settled_at || a.timestamp).getTime());
    let winStreak = 0;
    for (const t of sortedSettled) { if ((t.pnl ?? 0) > 0) winStreak++; else break; }

    const initialCapital = balanceData.portfolioValue - totalPnl;
    const pnlPct = initialCapital > 0 ? (totalPnl / initialCapital) * 100 : 0;

    return { totalPnl, pnlPct, winRate, best, worst, winStreak, recentlySettled: sortedSettled, settledCount: settledTrades.length };
  ...
  const openPositions = useMemo(() => {
    ...
    if (!agentIdsByExchange) return all;
    return all.filter((p) => agentIdsByExchange.has(p.agent_id));
  ...
  const unrealizedPnl = openPositions.reduce((s, p) => s + (p.pnl ?? 0), 0);
              ...
              {openToShow.map((pos, i) => (
                <div key={pos.id}>
                  <div className="grid items-center py-6 text-[14px] cursor-pointer hover:bg-white/[0.02]"
                    ...
                    <span className="text-white tabular-nums">{formatCurrency(pos.total_cost)}</span>
                    <span className="text-[#919fa6] truncate">{pos.agent_name}</span>
                  </div>
                ...
                return (
                  <div key={t.id}>
                    <div className="grid items-center py-6 text-[14px] cursor-pointer hover:bg-white/[0.02]"
                      ...
                      <span className="text-[#919fa6]">{shortDate(t.settled_at || t.timestamp)}</span>
                    </div>
