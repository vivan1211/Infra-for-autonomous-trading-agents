"""Tail Buyer Kalshi Bot — Mechanical Tail-Buying Pipeline.
...
async def run_cycle(config: Config) -> dict:
    ...
    logger.info("Fetching tail-priced markets from Kalshi...")
    client = KalshiClient(config)
    try:
        markets = await ingest.fetch_markets(config, client=client)
        ...
        for market in markets[:config.max_markets_per_cycle]:
            ...
            # Kalshi min order size is 1 contract (whole contracts)
            ...
            # Kalshi min order size is always 1 contract
            if count < 1:
                stats["trades_skipped"] += 1
                continue
...
async def main():
    ...
    mode = config.mode
    logger.info("Tail Buyer Kalshi Bot Starting")
