"""Tail Buyer Kalshi bot configuration — all settings in one place, all from env vars.

No AI fields, no kelly, no models, no research, no confidence thresholds.
Pure mechanical tail-buying strategy on Kalshi.
"""
...
class Config:
    ...
    min_volume: float = field(default_factory=lambda: float(os.environ.get("MIN_VOLUME_OVERRIDE", os.environ.get("MIN_VOLUME", "50000"))))
