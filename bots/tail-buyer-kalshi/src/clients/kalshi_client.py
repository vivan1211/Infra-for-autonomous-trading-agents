...
class KalshiClient:
    ...
    async def place_order(
        ...
        **kwargs,
        ...
        # Price in dollars — passed via kwargs from bot.py
        ...
        logger.info(f"Routing {action} {side} {ticker} x{count} @ ${price_dollars:.4f} through tail-buy intercept")
        async with httpx.AsyncClient(timeout=30.0) as http:
            resp = await http.post(
                f"{self._intercept_url}/api/intercept/tail-buy",
                json=payload,
