...
Returns a list of Market dicts, server-side filtered by volume and expiry.
No database, no SQLite — just fetch, filter, return.
"""
...
def _extract_title_date(title: str, now: datetime) -> Optional[datetime]:
    ...
    Returns a timezone-aware datetime if a date is found in the title, else None.
    Polymarket's endDate field is unreliable for multi-resolution markets,
    so we cross-check against dates mentioned in the title.
    """
    # Match patterns like "by April 30", "on March 30", "before June 1"
...
async def fetch_markets(config: Config) -> List[Market]:
    ...
    base_params = {
        ...
        # on date-filtered queries. We re-sort client-side by volume before returning.
        "order": "endDate",
        "ascending": "true",
        "limit": 1000,
    }
    if config.min_volume > 0:
        base_params["volume_num_min"] = config.min_volume

    # Fetch in 12-hour initial chunks; each adaptively subdivides on 1000-row cap
    initial_hours = 12
    total_hours = config.max_expiry_days * 24
    ...
    for m in raw_markets:
        ...
        try:
            ...
            # Title date used as sanity check for multi-resolution markets
            # where endDate is wrong (e.g. Borova April 30 reported as March 31).
            try:
            ...
            # Only override if title date is significantly later AND market endDate seems suspect.
            title_date = _extract_title_date(m.get("question", ""), now)
            if title_date:
                title_days = max(0, (title_date - now).total_seconds() / 86400)
                if title_days > days_to_expiry + 2:  # Title says 2+ days later than endDate
            ...
            # Skip markets with extreme prices (no edge possible)
    ...
    # Restore volume-descending order (server sort is endDate asc — see workaround above)
