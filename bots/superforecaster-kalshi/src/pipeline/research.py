...
Always runs before the Superforecaster analysis, regardless of which reasoning model
the user selected. Uses perplexity/sonar-reasoning-pro for built-in web search.
"""
...
from src.prompts import RESEARCH_SYSTEM, RESEARCH_USER
...
async def research_market(market: Market, config: Config) -> str:
    ...
    # Superforecaster config may not define research_timeout; fall back to 600s.
    timeout = getattr(config, "research_timeout", 600.0)
