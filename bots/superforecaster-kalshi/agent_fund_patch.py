"""Entry point for Superforecaster Kalshi bot.
...
def fetch_credentials(agent_id: str, bot_token: str):
    ...
    try:
        ...
        for key in ("KALSHI_API_KEY", "KALSHI_PRIVATE_KEY", "OPENROUTER_API_KEY"):
        ...
        if "config_json" in creds:
            config_json = creds["config_json"]
            if isinstance(config_json, dict) and config_json.get("model"):
                os.environ["SUPERFORECASTER_MODEL"] = config_json["model"]
                logger.info(f"Model from config: {config_json['model']}")
...
def run_trading_cycle(agent_id: str, bot_token: str):
    ...
    logger.info("Starting Superforecaster Kalshi trading cycle")
