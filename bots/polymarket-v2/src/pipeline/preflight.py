...
async def check_openrouter_model(model: str, api_key: str, timeout: float = 15.0) -> CheckResult:
    ...
    headers = {
        ...
        "HTTP-Referer": "https://arbiter.fund",  # matches production call header
    }
    last_err = None
    for attempt in (1, 2):
        try:
            ...
            # Surface a longer error excerpt so upstream provider errors are diagnosable
...
async def check_polymarket_order_manager_ready(config: Config, timeout: float = 12.0) -> CheckResult:
    """Probe the CLOB **order manager** (execution path), not the public market-data API.

    The existing check_polymarket_clob_api() hits the public /markets endpoint, which
    stays 200 even while the matching engine is restarting — so it CANNOT catch the
    HTTP 425 "order manager not ready" state that causes every order to fail. This
    check makes an authenticated, non-mutating get_open_orders call — the same order
    path that returns 425 during a scheduled restart — and DEFERS the cycle if the
    order manager is down, before we spend ~$3 on research.

    This is an OPTIMIZATION, not the safety net: a restart can still begin mid-cycle
    after this passes, which the worker's bounded in-loop retry (client.py) handles.
    Therefore only a definitive 425/post-only signal aborts the cycle; any other
    probe error soft-passes so we never abort on an unrelated CLOB blip.

    Only runs in live mode (no CLOB orders are placed in training/paper).
    """
    import os
    name = "Polymarket order manager (execution readiness)"
    pk = os.environ.get("POLYMARKET_PRIVATE_KEY", "")
    funder = os.environ.get("POLYMARKET_FUNDER_ADDRESS", "")
    if not pk:
        # credential check reports the missing key; nothing to probe here.
        return CheckResult(name, True, "skipped (no private key)")

    def _probe():
        """Runs in a thread (CLOB client construction + derive_api_key are blocking).
        Returns (state, detail) where state ∈ {'ready','restarting','unknown'}."""
        try:
            from src.clients.polymarket_client import PolymarketClient
        except Exception as e:  # pragma: no cover - import guard
            return ("unknown", f"import failed: {type(e).__name__}: {e}")
        try:
            client = PolymarketClient(private_key=pk, funder_address=funder)
        except Exception as e:
            return ("unknown", f"client init failed: {type(e).__name__}: {e}")
        raw = getattr(client, "_clob_client", None)
        if raw is None:
            return ("unknown", "CLOB client not initialized")
        # SDK v2 method is get_open_orders; tolerate get_orders alias defensively.
        fn = getattr(raw, "get_open_orders", None) or getattr(raw, "get_orders", None)
        if fn is None:
            return ("unknown", "SDK client exposes neither get_open_orders nor get_orders")
        try:
            fn()  # authenticated, non-mutating; raises on 425 during a restart
            return ("ready", "order manager accepting requests")
        except Exception as e:
            status = getattr(e, "status_code", "unset")
            msg = str(getattr(e, "error_msg", e)).lower()
            if status == 425 or "order manager not ready" in msg:
                return ("restarting", f"425 order manager not ready")
            if status == 503 and ("post-only" in msg or "post_only" in msg):
                return ("restarting", "503 post-only window after restart")
            return ("unknown", f"probe error (status={status})")

    try:
        state, detail = await asyncio.wait_for(asyncio.to_thread(_probe), timeout=timeout)
    except Exception as e:
        # Probe itself failed/timed out — inconclusive. Soft-pass; the worker's
        # in-loop retry is the actual guard for execution.
        return CheckResult(name, True, f"probe inconclusive ({type(e).__name__}) — soft-pass")
    ...
    if state == "restarting":
        return CheckResult(
            name, False, detail,
            "Polymarket matching engine is restarting (scheduled maintenance) — deferring "
            "this cycle; the next cron run retries. Check status.polymarket.com.",
        )
    # unknown → soft-pass (don't abort the cycle on an unrelated/auth blip)
    return CheckResult(name, True, f"inconclusive: {detail} — soft-pass (worker retry guards execution)")
...
async def run_preflight(config: Config) -> bool:
    ...
    # In live mode, also probe the order MANAGER (execution readiness) so a
    # matching-engine restart (HTTP 425) defers the cycle instead of failing
    # every order. (The public CLOB check above can't see a restart.)
    if getattr(config, "mode", "training") == "live":
        tasks.append(check_polymarket_order_manager_ready(config))
